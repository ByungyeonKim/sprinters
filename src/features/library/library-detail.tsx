...
const highlightedStepContentCache = new Map<string, { source: string; promise: Promise<string> }>();

function clampStepIndex(stepParam: string, maxStep: number) {
...
function setStepSearchParam(searchParams: URLSearchParams, stepIndex: number) {
  if (stepIndex === 0) {
...
function areSearchParamsEqualExceptStep(currentUrl: URL, nextUrl: URL) {
...
function getHighlightedStepContent(slug: string, stepIndex: number, content: string) {
...
function getDeferredStepIndices(currentStep: number, maxStep: number) {
...
type LibraryDetailLoaderData = {
  ...
  currentStep: number;
  eagerHighlightedStepContents: HighlightedStepContents;
  deferredStepIndices: number[];
  deferredHighlightedStepContentsPromise: Promise<HighlightedStepContents> | null;
};

export function meta({ data }: { data?: LibraryDetailLoaderData }) {
  if (!data?.tutorial) {
...
export async function loader({ params, request }: { params: { slug: string }; request: Request }) {
  ...
  const flatSessions: LibrarySession[] = [];
  const chapterMeta: LibraryChapter[] = [];
...
export default function LibraryDetail() {
  const loaderData = useLoaderData<LibraryDetailLoaderData>();
...
function LibraryDetailContent({
  ...
  const maxStep = useMemo(() => {
    const firstLockedIndex =
      tutorial.chapterMeta.find((chapter: LibraryChapter) => chapter.locked)?.startIndex ??
