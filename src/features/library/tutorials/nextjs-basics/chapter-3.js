...
${h2('server-action-security', '서버에서 실행된다는 것의 실질적 의미')}

<p>CSR 환경에서는 브라우저에서 JavaScript가 실행되기 때문에, API를 호출할 때 사용하는 키도 함께 클라이언트 코드에 포함될 수 있습니다. 무료 API라면 큰 문제가 없을 수도 있지만, 요청할 때마다 비용이 발생하는 유료 API라면 상황이 달라집니다. 배포된 사이트에 접속한 사용자는 개발자 도구를 통해 네트워크 요청이나 번들 코드를 확인할 수 있기 때문에, API 키가 노출되면 누구나 그 키를 사용할 수 있습니다. 이렇게 되면 의도하지 않은 사용량이 발생해 비용이 크게 늘어날 수 있습니다.</p>

<p>예를 들어, ChatGPT와 같은 AI 서비스를 만들기 위해 <a href="https://openai.com/ko-KR/api/" target="_blank">OpenAI API</a>를 사용해 글을 생성하는 기능을 만든다고 가정해 보겠습니다. 사용자가 버튼을 누르면 API를 호출해 답변을 받아오는 기능입니다. 만약 이 API 키가 브라우저 코드 안에 들어 있다면, 다른 사람이 개발자 도구에서 그 키를 확인하고 마음대로 요청을 보낼 수도 있습니다. 이 경우 실제 비용은 API 키를 발급한 계정에 청구됩니다.</p>

<p>Server Action은 이런 문제를 해결하는 방법 중 하나입니다. Server Action은 브라우저가 아니라 서버에서 실행되는 함수이기 때문에, API 키를 <code>process.env.OPENAI_API_KEY</code> 같은 환경 변수에 안전하게 저장해 두고 사용할 수 있습니다. 이 값은 서버 내부에서만 사용되고 브라우저로 전달되지 않습니다. 클라이언트에는 API 키가 아니라 서버에서 실행된 결과만 전달되기 때문에, 민감한 정보를 노출하지 않고도 외부 API를 사용할 수 있습니다.</p>
