...
export function findQuestionDetail(questionId) {
  return supabase
    .from('qna_questions')
    .select(detailQuery)
    .eq('id', questionId)
...
// qna_likes
export function findUserLike(questionId, userId) {
  return supabase
    .from('qna_likes')
    .select('id')
...
export function insertLike(questionId, userId) {
  return supabase
    .from('qna_likes')
    .insert({ question_id: questionId, user_id: userId });
}

export function deleteLike(questionId, userId) {
  return supabase
    .from('qna_likes')
    .delete()
    .eq('question_id', questionId)
